...
# Sensitivity for choice nudging prob increase
```{r}
#set.seed(1234) # set seed to ensure replicability
# Set random effect Q
#df$Q <- rep(c(-0.5,-0.4,-0.3,-0.2,-0.1),each = 3*3*10*T) # Let's help those with low thriftiness
...
cn <- seq(0,0.0333,0.0009)

rate_cn <- rep(NA,length(cn))
...
for (CN in cn){
  ...
  rate_cn[counter] <- sum(df$D)/dim(df)[1]
}
...
library(scales)
library(ggplot2) # Just for the alpha function to make transparent colors

par(mar = c(5.1, 4.1, 2.1, 2.1))
# 1. Create a scatter plot of the raw data points for Choice Nudging
plot(cn, rate_cn,
     pch = 19,        
     col = "black",   
     xaxt = 'n', # Suppress the default x-axis
     xlab = "Intensity of Choice-Nudging Policy", 
     ylab = "Predicted Take-up Rate",
     cex.lab = 1.5
     # main = "Effectiveness of Policy Interventions"
)

# 2. Create custom x-axis with percentage labels
axis(1, at = cn, labels = percent(cn))

# 3. Fit a linear model and calculate confidence intervals
cn_trend <- lm(rate_cn ~ cn)
newdata <- data.frame(cn = seq(min(cn), max(cn), length.out = 100))
conf_int <- predict(cn_trend, newdata = newdata, interval = "confidence", level = 0.95)

# 4. Plot the confidence interval as a transparent blue band
polygon(x = c(newdata$cn, rev(newdata$cn)), 
        y = c(conf_int[, "lwr"], rev(conf_int[, "upr"])),
        col = alpha("red", 0.2), border = NA)

# 5. Add the fitted trend line
abline(cn_trend, col = "red", lwd = 4)

# 6. Add the horizontal line for the Attention-Boosting policy
abline(h = rate_ab, col = "blue", lwd = 4, lty = 2)
...
# 8. Add a legend
legend("topleft", 
       legend = c("Choice-Nudging (Data)", 
                  "Choice-Nudging (Trend)", 
                  # "95% Confidence Interval",
                  "Attention-Boosting"
                  #"CI Intersection Points"
                  ),
