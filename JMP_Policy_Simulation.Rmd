...
N  = 450
T = 30
names_df <- c("id","month","NK1", "NK2to5", "kid1age", "kid2age", "YA", "edu", "LA", "Y", "logB", "Q","Pa", "Pc", "S", "pastD", "D")
K  = length(names_df)
...
# Generate YA
df$YA <- pmin(df$NK1, df$NK2to5, na.rm = TRUE)
...
Finkelstein and Notowidigto (2019) shows that the attention raising policy likely has an upper bound of 30% for SNAP.
We will use that as a benchmark for our counterfactual studies.

Attention raising policy:one shot increase in attention by 30% after exit the program
Choice inducing policy: increase choice probability by 3% for ten month when participating in the program and the child's age is between 6 to 15 month.

# Attention raising policy
  ...
  # attention raising policy intervention
    if (t >= 2){ # The intervention will only be triggered when a household exits WIC
      if (X$pastD[t-1] == 1 & X$pastD[t] == 0){
        Pai[t] < - Pai[t] + 0.3 # attention raise by 30%
        di[t] <- rbinom(1,1,min(Pai[t],1)) * rbinom(1,1,Pci[t]) 
        # print(c(i,t))
      } else {
        di[t] <- rbinom(1,1,Pai[t]^(1-d0)) * rbinom(1,1,Pci[t])
      }
...
for (i in 1:N){ # loop through all households
  ...
  # attention raising policy intervention
    if (X$pastD[t] == 1 & X$YA[t] > 10 & X$YA[t] <= 20){ 
      # The intervention will only be triggered when the household is in the program AND 
      # YA is between the 11 to 20 month
      Pci[t] < - Pci[t] + 0.03 # choice raise by 3%
      di[t] <- rbinom(1,1,Pai[t]^(1-d0)) * rbinom(1,1,min(Pci[t],1))
    } else {
      di[t] <- rbinom(1,1,Pai[t]^(1-d0)) * rbinom(1,1,min(Pci[t],1))
    }
