...
# Combine data into a tidy data frame (required for legends in ggplot2)
Q_all <- data.frame(
  Q = c(Q_NI, Q_AR, Q_CI),
  Group = rep(c("NI", "AR", "CI"), 
              times = c(length(Q_NI), length(Q_AR), length(Q_CI)))
)
  
# Plot with legend
ggplot(Q_all, aes(x = Q, color = Group)) +
  geom_density(linewidth = 1.5) +
  scale_color_manual(
    values = c("NI" = "gray", "AR" = "blue", "CI" = "red"),
    name = ""  # Empty legend title (remove if you want no title)
  ) +
  ...
  guides(color = guide_legend(
    override.aes = list(
      ...
      # shape = NA,        # Remove points (set to `NA`)
      linewidth = 2    # Adjust line thickness in legend
    )
  ))
```
...
base_plot <- ggplot() +
  ...
  labs(x = "Random effect Q", y = "Density") +
  theme_minimal(base_size = 20) +
  xlim(-3.5, 2) + ylim(0, 0.8) +
  theme(legend.position = "none")

p0 <- base_plot +
  geom_density(
    data = subset(plot_data0, Group == "NI0"),
    aes(x = Q, color = Group),
    linewidth = 1.5
  ) +
  geom_density(
    data = subset(plot_data0, Group == "FN0"),
    aes(x = Q, color = Group),
    linewidth = 1.5
  )
